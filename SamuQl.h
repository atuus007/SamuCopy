...
//        action = argmax_ap_f ( prg, image );
    ...
    SPOTriplet exec ( SPOTriplet a ) {

        return prev_pres[a];

    }

    SPOTriplet operator() ( SPOTriplet triplet, std::string prg, int* pres, bool isLearning ) {
        ...
        double reward =
            //3.0*triplet.cmp ( prev_action ) - 1.5;
            ( triplet == exec ( prev_action ) ) ?max_reward:min_reward;
        ...
        */
        ...
        SPOTriplet action = triplet; // amig a 8 akció-nál kevesebb a cellák színe, megteszi, maradhat
        action = 4;
        ...
        if ( prev_reward >  -std::numeric_limits<double>::max() ) {
            ...
            if ( isLearning ) {
                ...
                for ( int i {0}; i<9; ++i ) {
                    table_[i][prg] = table_[i][prg];    // kell az a zöld
                }

                //table_[action][prg] = table_[action][prg]; // ne zöldüljön :)

                //table_[prev_action][prev_state] = table_[prev_action][prev_state];
        ...
        prev_reward = reward;   	// r <- r'
        prev_action = action;	// a <- a'
        std::memcpy ( prev_pres, pres, 9*sizeof ( int ) );
    ...
    double alpha ( int n ) {
        ...
        //return 1000.0/ ( ( ( double ) n ) + 800.0 );
        //return 750.0/ ( ( ( double ) n ) + 500.0 );
